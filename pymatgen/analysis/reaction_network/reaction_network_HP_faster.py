...
class ReactionNetwork(MSONable):
    ...
    def find_paths(self, starts, target, weight, num_paths=10, solved_PRs_path=None, ignorenode=[], filename=None):  # -> ??
        """
        ...
        if solved_PRs_path is None:
            ...
            if len(self.graph.nodes) == 0:
                self.build()
            t1 = time.time()
            PR_paths = self.solve_prerequisites(starts, weight, filename=filename)
        ...
        while len(paths) < num_paths and my_heapq:
            ...
            print(cost_HP, path_dict_HP_class.path)
            d.append(path_dict_HP_class.path)
