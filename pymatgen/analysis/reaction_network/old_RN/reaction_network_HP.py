...
class ReactionNetwork(MSONable):
    ...
    def solve_prerequisites(self, starts: List[int],target:int,  weight: str, max_iter=20, save=False,
        ...
        if filename is None:
            filename = ""
        dumpfn(PRs, "PRs_dump_before_finalPRCheck"+filename+".json", default=lambda o: o.as_dict)
        dumpfn(json_graph.adjacency_data(self.graph), "self_graph_finalPRcheck"+filename+".json")
        dumpfn(self.min_cost, "min_cost_before_finalPR_check"+filename+".json")
        self.final_PR_check(PRs)
        if save:
            if filename is None:
                print("Provide filename to save the PRs, for now saving as PRs.json")
                filename2 = "PRs_dump_after_finalPRCheck"+filename+".json"
    ...
    def find_paths(self, starts, target, weight, num_paths=10, min_cost_input = None, solved_PRs_path=None, filename=None):  # -> ??
        """
        ...
        if solved_PRs_path is None:
            self.min_cost = {}
            self.graph = self.build()
            PR_paths = self.solve_prerequisites(starts,target, weight, filename=filename)
            ...
            #self.min_cost = {}
            #for key in PR_paths:
            #    self.min_cost[int(key)] = 10000000000000000
            #    for start in PR_paths[key]:
            #        if self.min_cost[int(key)] == 10000000000000000:
            #            self.min_cost[int(key)] = PR_paths[key][start].cost
            #        elif self.min_cost[int(key)] > PR_paths[key][start].cost:
            #            self.min_cost[int(key)] = PR_paths[key][start].cost
            self.min_cost = {}
            for node in min_cost_input:
                 self.min_cost[int(node)] = min_cost_input[node]
            #self.build()
